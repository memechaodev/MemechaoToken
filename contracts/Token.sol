...
contract Token is Context, IERC20Metadata, Ownable {
  ...
  constructor() {
    _name = 'Memechao';
    _symbol = 'MCHAO';
    _mint(0x5DBD25596d3DEE58E0fD7BFd5986e6E5ED961F8A, presaleReserve);
  ...
  /**
   * @dev Change the receiver address for transaction taxes.
   * @param _transactionTaxReceiver The new address to receive transaction taxes.
   * Only the contract owner can execute this function.
   */
  function changeTransactionTaxReceiver(address _transactionTaxReceiver) external onlyOwner {
    address oldReceiver = transactionTaxReceiver;
    transactionTaxReceiver = _transactionTaxReceiver;
    emit TransactionTaxReceiverChanged(oldReceiver, _transactionTaxReceiver);
  }
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    uint256 transactionTax = isWhitelisted[_msgSender()] || isWhitelisted[recipient] ? 0 : ((amount * transactionTaxAmount) / 100);
    if (transactionTax > 0) _transfer(_msgSender(), transactionTaxReceiver, transactionTax);
    _transfer(_msgSender(), recipient, amount - transactionTax);
    return true;
  }
